...
**tibble** - special `tidyverse` form of a `dataframe`, functionally very similar but has some improvements in e.g. printing (compare `iris` vs `dplyr::as_tibble(iris)`). `readr::read_csv()` will return a `tibble`, `read.csv()` from base R will return a `data.frame`.
