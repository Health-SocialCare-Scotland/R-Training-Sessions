...
PM_Borders1 <- read_csv("H:/R Training Data/PM_Borders1.csv")

# Read in PM_Borders2.csv

PM_Borders2 <- read_csv("H:/R Training Data/PM_Borders2.csv")
```
...
4. Take all records in `joined_data` which have `ManagementofPatient` values "Day Case in Inpatient Ward" or "Day Case in Day Bed Unit" and store them in a dataframe called `Q4_data`. Calculate a new column `LOS2` for `LengthOfStay` multiplied by two.
