...
smr01 <- dbGetQuery(
  smra_connection,
  paste(
    "select DISCHARGE_DATE, HBTREAT_CURRENTDATE,",
    "LOCATION, MAIN_OPERATION",
    "from SMR01_PI",
    "where ROWNUM <= 100"
  )
)
...
Borders_filtered <- filter(Borders, 
                           HospitalCode == "B120H" & LengthOfStay > 10)
...
```{r,  eval=FALSE,echo=TRUE}
inner_join(x = dataframe1, y = dataframe2, by = "common_variable")
```
