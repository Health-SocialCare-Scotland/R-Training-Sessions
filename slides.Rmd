...
- Packages come with extensive helpfiles called vignettes. These contain information on all the functions in the package, a description of what the packaage is designed to do

- Link to package vignette - suggestions of a good one?
...
**Objects**<br/>
- can't remember what we were going to put here? Any suggestions welcome!
...
hospital_codes <- read_csv(
  ...
  cbd1802e-0e04-4282-88eb-d7bdcfb120f0/resource/
  c698f450-eeed-41a0-88f7-c1e40a568acc/download/
  current_nhs_hospitals_in_scotland_030320.csv")
```
...
The `table()` function can be used to create frequencies and crosstabs from your data. Using the following code will give a frrequency table for column 1: 
...
- The tidyverse is a collection of R packages for data exploration, manipulation and visualisation. The packages in the tidyverse use a consistent format and data structure which ensures they work well together and are simple to use. 

- In tidyverse functions, the first argument is the dataset e.g. `function(data, task)` which makes the code more readable and consistent. 
...
```{r,  eval=FALSE,echo=TRUE}
counts <- as.data.frame(table(borders_age$admissionday))
colnames(counts) <- c("day", "count")

# plot
bar_plot <- ggplot(counts, aes(x = day, y = count)) +
...
PHI R User Group Inbox: <nss.Rusergroupinbox@nhs.net> <br/>
